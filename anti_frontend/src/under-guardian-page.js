...
async function loadAll() {
  ...
  const [relationsPage, incomingAll, outgoingAll] = await Promise.all([
    listRelations("ward", currentPage, pageSize),
    listGuardianRequests("incoming", "all"),
    listGuardianRequests("outgoing", "all"),
  ]);
  const relations = Array.isArray(relationsPage?.items) ? relationsPage.items : [];
  totalItems = Number(relationsPage?.total || 0);
  totalPages = Math.max(Number(relationsPage?.total_pages || 1), 1);
  currentPage = Math.min(Math.max(Number(relationsPage?.page || 1), 1), totalPages);
  const requests = [
    ...incomingAll.map((r) => ({ ...r, _box: "incoming" })),
    ...outgoingAll.map((r) => ({ ...r, _box: "outgoing" })),
  ].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  tbody.innerHTML = "";
  relations.forEach((item, index) => tbody.appendChild(createRelationRow(item, index)));
  renderPagination(relations.length);
